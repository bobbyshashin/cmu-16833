...
def main():
    ...
    if test_raycasting:
        rays = sensor_model.rayCastingLookUp(np.array([2500, 6000, 0]))
        # rays = sensor_model.rayCasting(np.array([5160, 3967, 0]))
        print(rays)
        map_utils.visualizeRays([2500,6000,0], rays)
        plt.pause(100)
    ...
    for time_idx, line in enumerate(logfile):
        ...
        for m in range(0, num_particles):
            ...
            # z_expected_arr = sensor_model.rayCasting(x_t1)

            # map_utils.visualizeRays([x_t1[1], x_t1[0], x_t1[2]], z_expected_arr)
            # plt.pause(2)
            """
            SENSOR MODEL
            """
            if (meas_type == "L"):
                z_t = ranges
                print("sensor model")
                w_t, z_expected_arr = sensor_model.beam_range_finder_model(z_t, x_t1)
                # print(z_expected_arr)
