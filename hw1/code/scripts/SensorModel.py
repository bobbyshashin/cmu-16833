...
class SensorModel:
    ...
    def precomputeRayCasting(self):
        ...
        np.save("../data/raycasting_table.npy", all_readings)
        return
    ...
    def rayCastingLookUp(self, laser_pose_in_map):
        ...
        x = np.minimum(
            self.map_x-1, np.maximum(laser_pose_in_map[:, 0] / self.map_resolution, 0)).astype(int)
        y = np.minimum(
            self.map_y-1, np.maximum(laser_pose_in_map[:, 1] / self.map_resolution, 0)).astype(int)

        theta_start = np.degrees(
            laser_pose_in_map[:, 2] - (math.pi / 2.0)).astype(int)
    ...
    def probShort(self, z_t, z_expected):
        ...
        if z_t < 0:
            ...
            res[z_t <= z_expected] = self.lambda_short * np.exp(-self.lambda_short * z_t) / (
                1.0 - np.exp(-self.lambda_short * z_expected[z_t <= z_expected]))
            return res
