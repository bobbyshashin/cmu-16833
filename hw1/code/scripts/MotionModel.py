...
class MotionModel:
    ...
    def update_vec(self, u_t0, u_t1, x_t0):
        ...
        rot1_est = rot1 - \
            np.random.normal(scale=math.sqrt(
                self.a1*rot1*rot1 + self.a2*trans*trans), size=num_particles)
        trans_est = trans - np.random.normal(scale=math.sqrt(
            self.a3*trans*trans + self.a4*rot1*rot1 + self.a4*rot2*rot2), size=num_particles)
        rot2_est = rot2 - \
            np.random.normal(scale=math.sqrt(
                self.a1*rot2*rot2 + self.a2*trans*trans), size=num_particles)

        x = x_t0[:,0] + trans_est * np.cos(x_t0[:,2]+rot1_est)
        y = x_t0[:,1] + trans_est * np.sin(x_t0[:,2]+rot1_est)
        theta = x_t0[:,2] + rot1_est + rot2_est
        theta = Utils.trimTheta_vec(theta)
        # print("theta range: ", min(theta), max(theta))
